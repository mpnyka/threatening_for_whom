...
nuts_labels <- read_csv("~/Desktop/Sociology/Dissertation/Analysis/paper_3/nuts_labels.csv") %>%
...
nuts_evs_correspondence <- EVS2008 %>% 
  ...
  mutate(nuts2 = case_when(nuts2_name == "region de bruxelles-capitale/brussels hoofdstedelijk" ~ "BE10",
                           ...
                           TRUE ~ nuts2)) %>%
  select(id, nuts2) %>%
  write_csv("~/Desktop/Sociology/Dissertation/Analysis/EVS/nuts_evs_correspondence.csv")
